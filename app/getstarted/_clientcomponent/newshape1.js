...
export default function Newshape1() {
    ...
    const draw = () => {
        ...
        for (let i = 0; i < pts.length; i++) {
            ...
            ctx.strokeStyle = he === i ? 'yellow' : 'rgba(255,255,255,0.6)';
            ctx.lineWidth = he === i ? 4 : 2;
            ctx.stroke();
        }

        const handleRadius = Math.max(6, Math.min(12, Math.floor(canvas.width / 80)));
        for (let i = 0; i < pts.length; i++) {
            const p = pts[i];
            ctx.beginPath();
            ...
            <div style={{
                ...
                {lineData.map((line) => (
                    <div key={line.index} style={{ marginBottom: 2, fontFamily: 'monospace' }}>
                        <span style={{ fontWeight: 'bold' }}>Edge {line.index}:</span> ({line.p1.x}, {line.p1.y}) → ({line.p2.x}, {line.p2.y}) | <span style={{ color: '#0066cc', fontWeight: 'bold' }}>Length: {line.length}px</span>
                    </div>
                ))}
            </div>
